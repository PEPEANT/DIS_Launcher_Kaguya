...
export function showIntroScreen() {
  ...
  elements.gameScreen.dataset.mode = "playing";
  if (elements.snackRushPrestartOverlay) {
    elements.snackRushPrestartOverlay.hidden = true;
  }
}

export function setLobbyView(view = "home") {
  elements.lobbyScreen.dataset.lobbyView = view;
}

export function showLobbyScreen({ view = "home", mobilePanel = "none" } = {}) {
  ...
  elements.lobbyScreen.dataset.mobilePanel = mobilePanel;
  delete elements.lobbyScreen.dataset.entrySheet;
  delete elements.lobbyScreen.dataset.entryGame;
  setLobbyView(view);
}

export function showGameEntryScreen({
  gameId,
  mobilePanel = "none"
} = {}) {
  showLobbyScreen({
  ...
  if (gameId) {
    elements.lobbyScreen.dataset.entryGame = gameId;
  }
}

export function showGameScreen({ mode = "playing" } = {}) {
  elements.body.classList.remove("view-lobby", "view-intro");
  elements.body.classList.add("view-game");
  elements.introScreen.hidden = true;
  elements.lobbyScreen.hidden = true;
  elements.gameScreen.hidden = false;
  elements.gameScreen.dataset.mode = mode;
  if (elements.snackRushPrestartOverlay) {
    elements.snackRushPrestartOverlay.hidden = mode !== "prestart";
  }
}

export function setStartButtonState({ label, disabled }) {
  const labelTarget = elements.startButton?.querySelector(".snack-rush-entry-start-label");
  if (labelTarget) {
    labelTarget.textContent = label;
  } else if (elements.startButton) {
...
export function renderShopState({ balance = 0, isGuest = true, equippedSkin = "skin_0", ownedSkins = [] } = {}) {
  ...
  if (elements.shopGuestGate) {
    elements.shopGuestGate.hidden = !isGuest;
  }
...
export function setMobileNavState(activeKey = "home", { labelKey = null, labelText = "" } = {}) {
  const navButtons = [
    elements.mobileHomeButton,
    elements.mobileChatButton,
  ...
  if (elements.lobbyRouteLabel) {
    if (labelText) {
      elements.lobbyRouteLabel.textContent = labelText;
      delete elements.lobbyRouteLabel.dataset.i18n;
    } else {
      elements.lobbyRouteLabel.textContent = t(resolvedLabelKey);
      elements.lobbyRouteLabel.dataset.i18n = resolvedLabelKey;
    }
  }

  if (elements.lobbyRouteBadge) {
    const shouldShowSeasonBadge = activeKey === "chat";
    elements.lobbyRouteBadge.hidden = !shouldShowSeasonBadge;
    elements.lobbyRouteBadge.dataset.i18n = "ranking.currentSeasonBadge";
    if (shouldShowSeasonBadge) {
      elements.lobbyRouteBadge.textContent = t("ranking.currentSeasonBadge");
    }
  }
}
...
export function setMessageAlertState(visible = false) {
  [elements.profileMessagesButton, elements.desktopMessagesButton, elements.mobileAccountButton]
    .filter(Boolean)
    .forEach((element) => {
...
export function renderLobbyChatMessages(messages = [], { currentUid = "" } = {}) {
  ...
  messages.forEach((message) => {
    ...
    const isOwnMessage = currentUid && message.uid === currentUid;
    const isAdminNotice = message.messageType === "admin_notice";
    item.className = `chat-message-card${isOwnMessage ? " chat-message-card--own" : ""}${isAdminNotice ? " chat-message-card--admin" : ""}`;
    ...
    name.textContent = message.nicknameSnapshot || (isAdminNotice ? "ADMIN" : t("lobby.defaultNickname"));

    const badge = isAdminNotice ? document.createElement("span") : null;
    if (badge) {
    ...
    const title = isAdminNotice && message.title ? document.createElement("strong") : null;
    if (title) {
...
export function renderProfileSummary({
  ...
  if (elements.profileSeason1Period) {
  ...
  elements.profileHujupaySummary.hidden = !walletSummary;
}
